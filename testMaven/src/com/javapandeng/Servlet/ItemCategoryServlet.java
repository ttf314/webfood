...
public class ItemCategoryServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        ItemCategoryDao cl=new ItemCategoryDao();
        List<ItemCategory> itemCategories = new ArrayList<>();//数据集合
        ItemCategoryDao itemCategoryDao=new ItemCategoryDao();//操作
        if (name.equals("")){
            itemCategories = itemCategoryDao.getItemCategoryList();   // 获取搜索栏的食物数据
        }else {
            itemCategories = itemCategoryDao.getItemCategoryList2(name);   // 获取搜索栏的食物数据
        }

        //List<Meals> controlLines= cl.getMealsList();
        Gson gson=new Gson();//创建GSON对象
        String result=gson.toJson(itemCategories);//将查询结果对象转换成json格式字符串
