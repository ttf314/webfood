 ...
 */
@WebServlet("/MealsServlet4")
public class MealsServlet4 extends HttpServlet {

    private static final long serialVersionUID = 1L;

    public MealsServlet4() {
        super();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Meals meals = new Meals();//数据集合
        MealsDao mealsDao=new MealsDao();//操作
        meals = mealsDao.getMealsByname(name);   // 获取搜索栏的食物数据
        Gson gson=new Gson();//创建GSON对象
        String result=gson.toJson(meals);//将查询结果对象转换成json格式字符串
