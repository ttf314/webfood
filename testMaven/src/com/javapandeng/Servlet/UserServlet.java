...
public class UserServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        List<UserInfo> userInfoList = new ArrayList<>();//数据集合
        UserDao userDao=new UserDao();//操作
        userInfoList = userDao.getUserByUnameAndUpass(userString);   // 获取搜索栏的食物数据

        //List<Meals> controlLines= cl.getMealsList();
        Gson gson=new Gson();//创建GSON对象
        String result=gson.toJson(userInfoList);//将查询结果对象转换成json格式字符串
